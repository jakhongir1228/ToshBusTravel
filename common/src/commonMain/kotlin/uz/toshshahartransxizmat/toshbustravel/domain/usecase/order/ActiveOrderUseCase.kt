...
typealias ActiveOrderBaseUseCase = BaseUseCase<Unit, Flow<ActiveOrderData>>

class ActiveOrderUseCase: KoinComponent, ActiveOrderBaseUseCase {

    private val repository: NetworkRepository by inject()

    override suspend fun invoke(parameter: Unit): Flow<ActiveOrderData> {
        return repository.getActiveOrder()
    }

    suspend operator fun invoke(): Flow<ActiveOrderData>{
        return invoke(Unit)
    }
}
